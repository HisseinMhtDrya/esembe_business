...
if(isset($_GET['type']) && $_GET['type'] == 'membre') {
   if(isset($_GET['confirme']) && !empty($_GET['confirme'])) {
      ...
      $req = $bdd->prepare('UPDATE membre_esembe SET confirme = 1 WHERE id = ?');
      $req->execute(array($confirme));
   }

   if(isset($_GET['supprime']) && !empty($_GET['supprime'])) {
      $supprime = (int) $_GET['supprime'];
      $req = $bdd->prepare('DELETE FROM membre_esembe WHERE id = ?');
...
if(isset($_GET['search']) && !empty($_GET['search'])) {
   ...
   $membres = $bdd->query("SELECT * FROM membre_esembe WHERE nom LIKE '%$search%' OR prenom LIKE '%$search%' ORDER BY id DESC");
} else {
   $membres = $bdd->query('SELECT * FROM membre_esembe ORDER BY id DESC');
}
...
<body>
                ...
                <div class="row vh-80 bg-light rounded align-items-center justify-content-center mx-0">
                    <div class="col-md-10 text-center p-4">
                        ...
                        <div class="bg-light rounded h-100 p-4 table-responsive">
                            <h6 class="mb-4">Membres inscrits</h6>
      ...
      <li style="list-style:none;">
                                <tr>
                                    ...
                                    <td><?= $m['nom'] ?></td>
                                    <td><?= $m['prenom'] ?></td>
                                    <td><?php if($m['confirme'] == 0) { ?> 
          <a href="membres_insc.php?type=membre&confirme=<?= $m['id'] ?>" class="btn btn-primary">Confirmer</a><?php } ?> 
          <a href="membres_insc.php?type=membre&supprime=<?= $m['id'] ?>" class="btn btn-primary">Supprimer</a></li></td>      
                                </tr>
                                 <?php } ?>
                                </tbody>
                            </table>
