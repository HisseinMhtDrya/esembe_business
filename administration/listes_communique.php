...
session_start();
  ...
  $bdd = new PDO('mysql:host=127.0.0.1;dbname=math_stat_info;charset=utf8', 'root', '');

if(isset($_GET['type']) && $_GET['type'] == 'membre') {

   if(isset($_GET['supprime']) && !empty($_GET['supprime'])) {
      $supprime = (int) $_GET['supprime'];
      $req = $bdd->prepare('DELETE FROM communique WHERE id = ?');
...
if(isset($_GET['search']) && !empty($_GET['search'])) {
   ...
   $membres = $bdd->query("SELECT * FROM communique WHERE sujet LIKE '%$search%' OR contenu LIKE '%$search%' ORDER BY id DESC");
} else {
   $membres = $bdd->query('SELECT * FROM communique ORDER BY id DESC');
}
...
<body>
                ...
                <div class="row vh-80 bg-light rounded align-items-center justify-content-center mx-0">
                    <div class="col-md-10 text-center p-4">
                        ...
                        <div class="bg-light rounded h-100 p-4">
                            <h6 class="mb-4">listes communiqués</h6>
                            <?php if($membres->rowCount() > 0) { ?>
                            <table class="table table-striped">
                                <thead>
                                    <tr>
                                        <th scope="col">Id</th>
                                        <th scope="col">Sujte</th>
      ...
      <li style="list-style:none;">
                                <tr>
                                    ...
                                    <td><?= $m['contenu'] ?></td>
                                    <td><?= $m['redacteur'] ?></td>
                                    <td><?= $m['date_communique'] ?></td>
                                    <td>
          <a href="listes_communique.php?type=membre&supprime=<?= $m['id'] ?>" class="btn btn-primary">Supprimer</a></li></td>      
                                </tr>
                                 <?php } ?>
                                </tbody>
                            </table>
                            <?php } else { ?>
                            <p>Aucun communiqué trouvé</p>
                           <?php } ?>
                        </div>
   </div>
    </div>
                ...
                <div class="bg-light rounded-top p-4">
                    <div class="row">
                        <div class="col-12 col-sm-6 text-center text-sm-start">
                            &copy; <a href="htptps://www.college.com">MSI</a>, Tous droits réservés. <br>
                        </div>
