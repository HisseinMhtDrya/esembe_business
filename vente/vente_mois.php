...
$today = date('Y-m-d');
$vente = $bdd->prepare('SELECT * FROM vente WHERE MONTH(date_vente) = MONTH(NOW()) AND YEAR(date_vente) = YEAR(NOW()) ORDER BY id DESC');
$vente->execute();

$recup_vente_totale_mois = $bdd->prepare("SELECT SUM(montant_total) AS vente_totale FROM vente WHERE MONTH(date_vente) = MONTH(NOW()) AND YEAR(date_vente) = YEAR(NOW()) ");
          ...
          <div class="row justify-content-center">
           <div class="col-lg-12 col-md-12 align-items-center justify-content-center">
                        <div class="bg-white rounded h-100 p-4 table-responsive">
                            <h6 class="mb-4">Ventes du mois <span class="text-primary"><?=$vente_totale['vente_totale']  ?> Fc</span></h6>
