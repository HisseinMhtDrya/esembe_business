...
if(isset($_SESSION['id'])) {
   ...
   $requpdate = $bdd->prepare('UPDATE surho SET derniere_activite = NOW() WHERE id = ?');
   $requpdate->execute(array($_SESSION['id']));

   $requpdate = $bdd->prepare('UPDATE surho SET status = "En ligne" WHERE id = ?');
   $requpdate->execute(array($_SESSION['id']));

   $requserst = $bdd->prepare('SELECT * FROM surho WHERE id = ?');
   ...
   if($elapsed_time > 300) {
      $reqdisconnect = $bdd->prepare('UPDATE surho SET status = "Hors ligne" WHERE id = ?');
