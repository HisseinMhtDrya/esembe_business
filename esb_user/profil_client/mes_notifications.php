...
if(isset($_SESSION['id']) && $_SESSION['id'] > 0) {
    $confirme = 1;
    $req = $bdd->prepare('UPDATE notifications SET lu = 1 WHERE id_to = ?');
...
// Requête pour récupérer les notifications destinées à l'utilisateur en session
$sql = "SELECT notifications.*, surho.nom, surho.prenom, surho.avatar, surho.sexe FROM notifications 
 INNER JOIN surho ON notifications.id_from = surho.id WHERE id_to = :id_utilisateur AND surho.id<>:id_utilisateur ORDER BY id DESC";
...
if(isset($_GET['type']) && $_GET['type'] == 'membre') {
    ...
    if(isset($_GET['supprime']) && !empty($_GET['supprime'])) {
       ...
       $req = $bdd->prepare('DELETE FROM notifications WHERE id = ?');
       $req->execute(array($supprime));
       header('location:mes_notifications.php?id='.$_SESSION['id']);
    }
}
...
<a href=".../profil_membre_rud.php?id=<?php echo $_SESSION['id']; ?>">
...
<nav class="header-nav ms-auto">
  <ul class="d-flex align-items-center">
    ...
    <li class="nav-item dropdown">
      ...
      if(isset($_SESSION['id'])){
      ?>
      <a class="nav-link nav-icon" href="../profil_membre_rud.php?id=<?php echo $_SESSION['id']; ?>">
      <?php
      } else{
      ?>
       <a class="nav-link nav-icon" href="../../index.php">
      <?php
      }
      ?>
...
?>  
         ...
         ?>
                    ...
                    <div class="notification-list_detail">
                        <p><b><?=$expediteur ?><br> </b><?= $message ?></p>
                        <a href="">
                        <p class="text-info"><?=$contenu ?></p>
                        <p class="text-info"><small><?= $date_message ?></small></p>
                        </a>
                        <?php
        if($lu == 0) {
            ?>
            ...
            <a href="?type=membre&supprime=<?=$id_notification ?>" class="btn btn-outline-danger"><i class="bi bi-trash"></i></a>
            <?php
           }
             ?>
                    </div>
